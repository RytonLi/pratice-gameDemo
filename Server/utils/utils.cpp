...
std::string convertToUTF8(std::string str) {
    ...
    std::wstring_convert<std::codecvt_byname<wchar_t, char, mbstate_t>> Conver_GBK(new std::codecvt_byname<wchar_t, char, mbstate_t>(GBK_LOCALE_NAME)); //GBK - wchar_t

    std::wstring _str = Conver_GBK.from_bytes(str);

    std::wstring_convert<std::codecvt_utf8<wchar_t>> conv;

    return conv.to_bytes(_str);
...
bool sendData(uv_stream_t* stream, const char* data, int len) {
    ...
    if (uv_write(req, stream, &(req->buf), 1, on_write) != 0) {
        goto Exit0;
    }
    req = nullptr;
