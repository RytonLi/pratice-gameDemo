...
bool OnCloseClient(uv_tcp_t* client) {
    g_saveMgr.on_client_close(client);
    g_playerMgr.on_client_close(client);
    g_shopMgr.on_client_close(client);
    g_statusMgr.on_client_close(client);
    g_bagMgr.on_client_close(client);
...
bool _OnPackHandle(uv_tcp_t* client, Packet* pack) {
    ...
    switch (pack->cmd) {
        ...
        case CLIENT_SHOPCOMPLETE_RSP:
        {
            ShopCompleteRsp rsp;
            rsp.ParseFromArray(pack->data, pack->len);
            g_shopMgr.on_shopComplete_rsp(client, &rsp);
            break;
        }
        case CLIENT_BUY_REQ:
        {
            PlayerBuyReq req;
            req.ParseFromArray(pack->data, pack->len);
            g_shopMgr.on_buy_req(client, &req);
            break;
        }
        case CLIENT_BAGUPDATE_REQ: 
        {
            PlayerBagUpdateReq req;
            req.ParseFromArray(pack->data, pack->len);
            g_bagMgr.on_updateBag_req(client, &req);
            break;
        }
        case CLIENT_MONEY_REQ:
        {
            PlayerMoneyReq req;
            req.ParseFromArray(pack->data, pack->len);
            g_bagMgr.on_money_req(client, &req);
            break;
        }
        case CLIENT_STATUS_REQ: 
        {
            PlayerStatusReq req;
            req.ParseFromArray(pack->data, pack->len);
            g_statusMgr.on_status_req(client, &req);
            break;
        }
        default:
